...
const  MOVABLE_CHARACTER_LEFT:char = '[';
const MOVABLE_CHARACTER_RIGHT:char = ']';
...
fn is_movement_possible(robot_location:&Location, game_map:&TwoDimFromFile, move_x:&i32, move_y:&i32)->bool
{
    ...
    //println!("Origin point is [{}, {}], char here is {}", previous_point_y, previous_point_x,game_map.contents[previous_point_y][previous_point_x]);
    ...
    //println!("Next point is [{}, {}], char here is {}", move_origin_point_y, move_origin_point_x,game_map.contents[move_origin_point_y][move_origin_point_x]);
    ...
    else if next_block == MOVABLE_CHARACTER_LEFT ||  next_block == MOVABLE_CHARACTER_RIGHT //MOVABLE_CHARACTER
    {
        if *move_y == 0 // Moving horizontally 
        {
            return is_movement_possible(&Location { x: (move_origin_point_x), y: (move_origin_point_y) }, game_map, move_x, move_y);
        }
        else if *move_x == 0 // Moving vertically
        {
            let bool1 = is_movement_possible(&Location { x: (move_origin_point_x), y: (move_origin_point_y) }, game_map, move_x, move_y);
            let mut bool2 = false;
            if next_block == MOVABLE_CHARACTER_LEFT
            {
                bool2 = is_movement_possible(&Location { x: (move_origin_point_x+1 as usize), y: (move_origin_point_y) }, game_map, move_x, move_y);
            }
            else if next_block == MOVABLE_CHARACTER_RIGHT
            {
                bool2 = is_movement_possible(&Location { x: (move_origin_point_x-1 as usize), y: (move_origin_point_y) }, game_map, move_x, move_y);
            }
...
fn collect_all_movable_entities(robot_location:&Location, game_map:&TwoDimFromFile, move_x:&i32, move_y:&i32)->Vec<(Location, char)>
{
    ...
    movable_object_list.push( (Location{ x: robot_location.x, y: robot_location.y },  game_map.contents[ robot_location.y][ robot_location.x]));
    ...
    loop
    {
        ...
        else if next_block == MOVABLE_CHARACTER_LEFT //MOVABLE_CHARACTER
        {
            if *move_y == 0 // Moving horizontally 
            {
                movable_object_list.push( (Location { x:move_origin_point_x, y:move_origin_point_y}, MOVABLE_CHARACTER_LEFT));
            ...
            else if *move_x == 0 // Moving vertically
            {
                ...
                for elements in collect_all_movable_entities(&Location { x:move_origin_point_x+1 as usize, y:move_origin_point_y}, game_map, move_x, move_y)
                {
                    movable_object_list.push(elements);
                }
            }
            else {
                continue;
        ...
        else if next_block == MOVABLE_CHARACTER_RIGHT //MOVABLE_CHARACTER
        {
            if *move_y == 0 // Moving horizontally 
            {
                movable_object_list.push( (Location { x:move_origin_point_x, y:move_origin_point_y}, MOVABLE_CHARACTER_RIGHT));
            }
            else if *move_x == 0 // Moving vertically
            {
                movable_object_list.push( (Location { x:move_origin_point_x, y:move_origin_point_y}, MOVABLE_CHARACTER_RIGHT));
...
fn calculate_score(game_map:&TwoDimFromFile)->i32
{
    ...
    for(index_row, each_row) in game_map.contents.iter().enumerate()
    {
        ...
        {
            if *element == MOVABLE_CHARACTER_LEFT
            {
