...
use crate::{EMPTY_SPACE_CHARACTER, MOVABLE_CHARACTER, MOVABLE_CHARACTER_LEFT, MOVABLE_CHARACTER_RIGHT, ROBOT_CHARACTER, WALL_CHARACTER};
...
impl TwoDimFromFile
{
    ...
    pub fn new()-> Self
    {
      let contents:Vec<Vec<char>> = read_from_2d_file();
      Self{contents:resize_map_with_new_chars(contents)}
    }
...
fn resize_map_with_new_chars(game_map:Vec<Vec<char>>)->Vec<Vec<char>>
{
  let mut game_map_resized:Vec<Vec<char>> = Vec::new();

  for each_row in game_map.iter()
  {
    let mut new_row:Vec<char> = Vec::new();
    for row_char in each_row.iter()
      {
          if *row_char == WALL_CHARACTER
          {
              new_row.push(WALL_CHARACTER);
              new_row.push(WALL_CHARACTER);
          }
          else if *row_char == MOVABLE_CHARACTER
          {
              new_row.push(MOVABLE_CHARACTER_LEFT);
              new_row.push(MOVABLE_CHARACTER_RIGHT);
          }
          else if *row_char == EMPTY_SPACE_CHARACTER
          {
              new_row.push(EMPTY_SPACE_CHARACTER);
              new_row.push(EMPTY_SPACE_CHARACTER);
          }
          else if *row_char == ROBOT_CHARACTER
          {
              new_row.push(ROBOT_CHARACTER);
              new_row.push(EMPTY_SPACE_CHARACTER);
          }
          else {
              continue;
